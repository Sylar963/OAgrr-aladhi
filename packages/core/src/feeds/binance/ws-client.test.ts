...
describe('BinanceWsAdapter', () => {
  ...
  it('removes tracked streams even when unsubscribe happens while disconnected', async () => {
    const adapter = new BinanceWsAdapter();
    const internals = adapter as unknown as BinanceWsAdapterInternals;
    ...
    internals.subscriptions.subscribedStreams.add('btcusdt@optionMarkPrice');
