...
import { PaperOrderLegSchema, PlaceOrderRequestSchema, CreatePaperTradeRequestSchema, CreatePaperTradeNoteRequestSchema, ReducePaperTradeRequestSchema, InitPaperAccountRequestSchema, } from './paper.js';
const validLeg = {
    index: 0,
    side: 'buy',
    optionRight: 'call',
...
describe('PaperOrderLegSchema', () => {
    ...
    it('rejects an unknown venue in preferredVenues', () => {
        expect(PaperOrderLegSchema.safeParse({ ...validLeg, preferredVenues: ['kraken'] }).success).toBe(false);
    });
});
describe('PlaceOrderRequestSchema', () => {
    ...
    it('defaults venueFilter to an empty array when omitted', () => {
        ...
        expect(result.success).toBe(true);
        if (result.success)
            expect(result.data.venueFilter).toEqual([]);
    });
    ...
    it('strips an unexpected index key from an order leg', () => {
        ...
        expect(result.success).toBe(true);
        if (result.success)
            expect('index' in result.data.legs[0]).toBe(false);
    });
});
describe('CreatePaperTradeRequestSchema', () => {
    ...
    it('rejects a thesis over the 2000-char limit', () => {
        expect(CreatePaperTradeRequestSchema.safeParse({ order, thesis: 'x'.repeat(2_001) }).success).toBe(false);
    });
});
describe('CreatePaperTradeNoteRequestSchema', () => {
    ...
    it('rejects more than 12 tags', () => {
        ...
        expect(CreatePaperTradeNoteRequestSchema.safeParse({ kind: 'note', content: 'x', tags }).success).toBe(false);
    });
    it('rejects an unknown note kind', () => {
        expect(CreatePaperTradeNoteRequestSchema.safeParse({ kind: 'rumor', content: 'x' }).success).toBe(false);
    });
});
