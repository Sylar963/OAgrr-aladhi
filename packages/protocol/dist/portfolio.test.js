...
import { PositionLegSchema, VolShockScenarioSchema, PortfolioWsClientMessageSchema, PortfolioWsServerMessageSchema, } from './portfolio.js';
const validLeg = {
    legId: 'leg-1',
    underlying: 'BTC',
    expiry: '2026-03-27',
...
describe('PositionLegSchema', () => {
    it('round-trips a valid leg unchanged', () => {
        ...
        if (result.success)
            expect(result.data).toEqual(validLeg);
    });
    it('defaults realizedPnlUsd to 0 when omitted', () => {
        ...
        expect(result.success).toBe(true);
        if (result.success)
            expect(result.data.realizedPnlUsd).toBe(0);
    });
    ...
    it('accepts a null venueHint and null entryIv', () => {
        expect(PositionLegSchema.safeParse({ ...validLeg, venueHint: null, entryIv: null }).success).toBe(true);
    });
});
describe('VolShockScenarioSchema', () => {
    it('accepts each scenario kind', () => {
        ...
        expect(VolShockScenarioSchema.safeParse({ kind: 'skew_tilt', atmStrike: 70_000, slopePerLogK: 0.1 })
            .success).toBe(true);
        expect(VolShockScenarioSchema.safeParse({ kind: 'term_twist', pivotDays: 30, slopePerYear: 0.2 })
            .success).toBe(true);
        expect(VolShockScenarioSchema.safeParse({
            kind: 'atm_bump',
            atmStrike: 70_000,
            widthPct: 0.1,
            bumpVolPts: 3,
    ...
    it('rejects a skew_tilt with a non-positive atmStrike', () => {
        expect(VolShockScenarioSchema.safeParse({ kind: 'skew_tilt', atmStrike: 0, slopePerLogK: 0.1 })
            .success).toBe(false);
    });
});
describe('PortfolioWsClientMessageSchema', () => {
    it('accepts subscribe and unsubscribe', () => {
        expect(PortfolioWsClientMessageSchema.safeParse({ type: 'subscribe', subscriptionId: 's1' }).success).toBe(true);
        expect(PortfolioWsClientMessageSchema.safeParse({ type: 'unsubscribe', subscriptionId: 's1' }).success).toBe(true);
    });
    it('rejects an unknown message type', () => {
        expect(PortfolioWsClientMessageSchema.safeParse({ type: 'ping', subscriptionId: 's1' }).success).toBe(false);
    });
});
describe('PortfolioWsServerMessageSchema', () => {
    it('accepts a hello message', () => {
        expect(PortfolioWsServerMessageSchema.safeParse({ type: 'hello', accountId: 'a1', serverTime: 1_000 })
            .success).toBe(true);
    });
    it('accepts an error message', () => {
        expect(PortfolioWsServerMessageSchema.safeParse({ type: 'error', code: 'BOOM', message: 'bad' })
            .success).toBe(true);
    });
    it('rejects a hello with a non-integer serverTime', () => {
        expect(PortfolioWsServerMessageSchema.safeParse({ type: 'hello', accountId: 'a1', serverTime: 1.5 })
            .success).toBe(false);
    });
});
