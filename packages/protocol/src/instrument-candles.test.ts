...
describe('InstrumentCandleSchema', () => {
  it('round-trips a valid candle unchanged', () => {
    ...
    if (result.success) expect(result.data).toEqual(validCandle);
  });
...
describe('InstrumentCandlesResponseSchema', () => {
  ...
  it('rejects an unknown venue', () => {
    expect(InstrumentCandlesResponseSchema.safeParse({ ...validResponse, venue: 'kraken' }).success).toBe(
      false,
    );
  });

  it('rejects an empty or over-long priceCurrency', () => {
    expect(InstrumentCandlesResponseSchema.safeParse({ ...validResponse, priceCurrency: '' }).success).toBe(
      false,
    );
    expect(
      InstrumentCandlesResponseSchema.safeParse({ ...validResponse, priceCurrency: 'TOOLONGCUR' })
        .success,
    ).toBe(false);
  });
});
...
describe('InstrumentCandlesQuerySchema', () => {
  ...
  it('round-trips a valid query unchanged', () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) expect(result.data).toEqual(validQuery);
  });
