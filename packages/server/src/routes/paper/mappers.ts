...
export function orderToDto(order: Order): PaperOrderDto {
  return {
    ...
    status: order.status,
    legs: order.legs.map((leg) => ({ ...leg, quantityUnit: 'base' })),
...
export function fillToDto(fill: Fill): PaperFillDto {
  return {
    ...
    quantityUnit: 'base',
    contractMultiplierBase: fill.contractMultiplierBase ?? null,
    nativeQuantity: fill.nativeQuantity ?? null,
    requestedNativeQuantity: fill.requestedNativeQuantity ?? null,
    nativeMinQuantity: fill.nativeMinQuantity ?? null,
    nativeQuantityStep: fill.nativeQuantityStep ?? null,
    nativePriceTick: fill.nativePriceTick ?? null,
