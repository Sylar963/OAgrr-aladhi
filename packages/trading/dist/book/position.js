export function positionKeyId(key) {
    return `${key.accountId}|${key.underlying}|${key.expiry}|${key.strike}|${key.optionRight}`;
}
export function keyFromFill(accountId, fill) {
    return {
        accountId,
 ...
 */
export function applyFillToPosition(prior, fill) {
    ...
    if (sameDirection) {
        ...
        const weightedAvg = (prior.avgEntryPriceUsd * Math.abs(prior.netQuantity) +
    ...
    const realizedDelta = priorSign * closingQty * (fill.priceUsd - prior.avgEntryPriceUsd);
