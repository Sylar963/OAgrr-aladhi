import { InsufficientMarginError, InvalidOrderError, NoLiquidityError, TradingError, } from '../book/errors.js';
import { fillCashDelta } from '../book/fill.js';
import { newClientOrderId, newOrderId, } from '../book/order.js';
...
export class OrderPlacementService {
    ...
    marginEngine;
    pnlService;
    constructor(orders, positions, fillEngine, clock, options = {}) {
        this.orders = orders;
    ...
    async checkMargin(order, venueFilter) {
        if (this.marginEngine instanceof NoopMarginEngine)
            return;
        ...
        const prospectiveLegs = order.legs.map((l) => ({
            index: l.index,
            side: l.side,
        ...
        await this.orders.updateOrderStatus(rejected);
        throw new InsufficientMarginError(reason, result.requiredUsd, result.availableUsd, result.bufferUsd);
    }
    async equityFor(accountId) {
        if (this.pnlService) {
