...
export class ApproximationMarginEngine {
    quotes;
    opts;
    ...
    async estimate(input) {
        ...
        for (const leg of input.prospectiveLegs) {
            ...
            if (spot == null) {
                throw new MarginCheckUnavailableError(`Cannot estimate margin for leg ${leg.index}: spot price unavailable`, leg.index, 'spot_unavailable');
            }
    ...
    async spotForLeg(key, venueFilter) {
        const books = await this.quotes.getBooks(key, venueFilter);
        for (const b of books) {
            if (b.underlyingPriceUsd != null && b.underlyingPriceUsd > 0)
                return b.underlyingPriceUsd;
        }
        return null;
    }
    async existingShortMargin(positions, venueFilter) {
        let total = 0;
...
function perContractShortMargin(optionRight, strike, spot, opts) {
    const otm = optionRight === 'call' ? Math.max(0, strike - spot) : Math.max(0, spot - strike);
    const reference = optionRight === 'put' ? strike : spot;
    return Math.max(opts.k1 * spot - otm, opts.k2 * reference);
}
//# sourceMappingURL=approximation-margin-engine.js.map