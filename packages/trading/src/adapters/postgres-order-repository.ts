...
function fromOrderRow(row: PaperOrderRow): Order {
  return {
    ...
    kind: row.kind,
    status: row.status,
    legs: ((row.legs as Array<Omit<OrderLeg, 'quantityUnit'>>) ?? []).map((leg) => ({
      ...leg,
      quantityUnit: 'base',
    })),
...
function toFillRow(fill: Fill): PaperFillRow {
  return {
    ...
    quantityUnit: fill.quantityUnit,
    contractMultiplierBase: fill.contractMultiplierBase,
    nativeQuantity: fill.nativeQuantity,
    requestedNativeQuantity: fill.requestedNativeQuantity,
    nativeMinQuantity: fill.nativeMinQuantity,
    nativeQuantityStep: fill.nativeQuantityStep,
    nativePriceTick: fill.nativePriceTick,
...
function fromFillRow(row: PaperFillRow): Fill {
  return {
    ...
    contractMultiplierBase: row.contractMultiplierBase,
    nativeQuantity: row.nativeQuantity,
    requestedNativeQuantity: row.requestedNativeQuantity,
    nativeMinQuantity: row.nativeMinQuantity,
    nativeQuantityStep: row.nativeQuantityStep,
    nativePriceTick: row.nativePriceTick,
