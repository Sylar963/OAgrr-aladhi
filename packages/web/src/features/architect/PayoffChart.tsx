...
export default function PayoffChart({
  ...
  const draw = useCallback((hoverInfo: HoverInfo | null = null, dragState: DragState | null = null) => {
    ...
    for (const strike of uniqueStrikes) {
      ...
      const labels = legsAtStrike.map((l) =>
  ...
  const findNearestStrike = useCallback((px: number): number | null => {
    const layout = getLayout();
  ...
  const findHandleAtPos = useCallback((mx: number, my: number): { legId: string; strike: number } | null => {
    const layout = getLayout();
    if (!layout) return null;
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (handle) {
      ...
      setDrag({ legId: handle.legId, legIndex: legIdx, startX: mx, currentStrike: handle.strike });
    }
  ...
  const handleMouseUp = useCallback(() => {
    if (drag && onLegStrikeDrag && drag.currentStrike !== dataRef.current.legs[drag.legIndex]?.strike) {
